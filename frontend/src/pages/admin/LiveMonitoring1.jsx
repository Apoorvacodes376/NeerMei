...
export default function LiveMonitoring1() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await invokeMl('pre', 'train/start', { mode: 'dummy' })
      await invokeMl('pre', 'buffer/batch', { mode: 'dummy', entries: records.map(({ sensorValues, label }) => ({ sensorValues, label })) })
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          {!connected && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--danger)', fontSize: '0.8rem' }}>
              <WifiOff size={16} /> Hardware disconnected
            </div>
          )}
      ...
      <LiveGraph data={rows.slice().reverse()} metrics={['pH', 'turbidity']} title="pH & Turbidity — Live" />
...
function toRow(reading) {
  return { timestamp: reading.created_at, ...reading.sensor_values }
}
